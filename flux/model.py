...
class Flux(nn.Module):
    ...
    def forward(
        ...
        emostyle_inputs: Tensor | None = None,
    ) -> Tensor:
        ...
        if self.feature_embedder is not None and siglip_inputs is not None and len(siglip_inputs) > 0 and self.vision_encoder is not None:
            ...
            print("use siglip multi-feat inputs")
        ...
        if emostyle_inputs is not None:
            txt = torch.cat((emostyle_inputs, txt), dim=1)
            emostyle_embed_ids = torch.zeros(
                emostyle_inputs.shape[0], emostyle_inputs.shape[1], 3
            ).to(txt_ids.device)
            txt_ids = torch.cat((emostyle_embed_ids, txt_ids), dim=1)
